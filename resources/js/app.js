...
return text==null ||undefined||'' ? '' :text.charAt(0).toUpperCase() + text.slice(1);
});
...
Vue.filter('singularize', (text)=>{
    ...
    if(text.substr(text.length-3, text.length) == 'ies'){
        //categories
...
const app = new Vue({
    ...
    data:{
        records: [
            10,5,20,30,40,50,60,70,80,90,100,200
            ],
    logs:{
    ...
    methods:{
        ...
        generatePdf(options){
            ...
            let docDefinition = {
                ...
                content:[
                    {
                        stack:[
                            ...
                            {text:this.settings.sch_name.toUpperCase(), style:['mainHeadings']},
                            {text:"OFFICE OF THE REGISTRAR ACADEMIC AFFAIRS AND RESEARCH", style:'mainHeadings'},
                            {text: this.settings.po_address, style:'mainHeadings'},
                            {text:`Telephone: ${this.settings.phone}, Email: ${this.settings.email.toLowerCase()}`,margin:[0,0,0,15], style: ['mainHeadings']}
                        ]
                    },
                    {
                        table:{
                ...
                footer:[
                    {text: `${this.settings.sch_name.split(" ").map(name=>name.charAt(0).toUpperCase()+name.substr(1, name.length)).join(" ")} - ${this.settings.sch_motto}`, style:'footer'}
                ]
            }
    //make the pdf document
    pdfMake.createPdf(docDefinition).download(options.filename || 'untitled.pdf');
    console.log("Finished creating the new pdf document for you!!")
        },
        generateExcel(options){
            let wb = XLSX.utils.table_to_book(document.querySelector(options.table), {sheet:"School Management System"});
            ...
            //save excel file
            FileSaver.saveAs(new Blob([s2ab(wbout)], {type:'application/octet-stream'}), options.filename || 'untitled.xlsx');
        },
        generateCsv(options){
            let wb = XLSX.utils.table_to_book(document.querySelector(options.table), {sheet: 'School Management System'});
            let ws1 = wb.Sheets[wb.SheetNames[0]];
            let csv = XLSX.utils.sheet_to_csv(ws1, {strip: true});
            FileSaver.saveAs(new Blob([csv], {type:'text/csv'}), options.filename || 'untitled.csv');
            console.log("Has finished creating the csv file for you");
        }
    },
